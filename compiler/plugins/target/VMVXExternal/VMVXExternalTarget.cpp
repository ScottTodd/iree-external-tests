...
getVMVXExternalExecutableTarget(bool enableMicrokernels, MLIRContext *context,
...
class VMVXExternalTargetBackend final : public TargetBackend {
public:
  VMVXExternalTargetBackend(const VMVXExternalOptions &options) : options(options) {}

  std::string name() const override { return "vmvx-external"; }
...
private:
  ArrayAttr getExecutableTargets(MLIRContext *context) const {
    ...
    targetAttrs.push_back(getVMVXExternalExecutableTarget(
        options.enableMicrokernels, context, "vmvx", "vmvx-bytecode-fb"));
    return ArrayAttr::get(context, targetAttrs);
  }

  const VMVXExternalOptions &options;
};

namespace {
struct VMVXExternalSession
    : public PluginSession<VMVXExternalSession, VMVXExternalOptions,
                           PluginActivationPolicy::DefaultActivated> {
  void populateHALTargetBackends(IREE::HAL::TargetBackendList &targets) {
    // #hal.device.target<"vmvx", ...
    // #hal.executable.target<"vmvx", ...
    targets.add("vmvx-external",
                [&]() { return std::make_shared<VMVXExternalTargetBackend>(options); });
  }
};
...
extern "C" bool iree_register_compiler_plugin_hal_target_vmvx_external(
    mlir::iree_compiler::PluginRegistrar *registrar) {
  registrar->registerPlugin<mlir::iree_compiler::IREE::HAL::VMVXExternalSession>(
      "hal_target_vmvx_external");
  return true;
}

IREE_DEFINE_COMPILER_OPTION_FLAGS(mlir::iree_compiler::IREE::HAL::VMVXExternalOptions);
